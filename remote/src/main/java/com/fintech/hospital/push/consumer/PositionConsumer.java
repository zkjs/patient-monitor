...
public class PositionConsumer implements PushConsumer {
  ...
  public void consume(String msg) {
    ...
    try {
      ...
      List<TimedPosition> positions = recordTrace(ap, apMsg, braceletId);
      /* based on ap signals, try to locate the patient */
      TimedPosition pos = position(positions, braceletId);

      //TODO push the position
      if (pos != null) pushService.notifyPosition(new PushMsg(posChannel4App, JSON.toJSONString(pos)));
...
//                  positioning(positions, braceletId, null, USE_EUCLIDEAN) :
  ...
  private List<TimedPosition> recordTrace(AP ap, APMsg apMsg, String braceletId) {
    ...
    return cache.push(braceletId, ap.getAlias(), new TimedPosition(ap, System.currentTimeMillis(), apMsg.getRssi()));
  }
